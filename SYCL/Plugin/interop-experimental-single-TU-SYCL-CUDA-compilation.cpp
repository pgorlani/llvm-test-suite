...
template<typename T>
__device__ T test_cuda_function_0(T a, T b) {return a+b;}
template<typename T>
__host__ T test_cuda_function_0(T a, T b) {return -a-b;}

__device__ int test_cuda_function_1() {return blockIdx.x * blockDim.x + threadIdx.x;}

__device__ inline float test_cuda_function_2(float a, float b) {return -sin(a)+b;}

__device__ inline float test_cuda_function_3(float a, float b) {return sin(a)-b;}
__host__ inline float test_cuda_function_3(float a, float b) {return 0;}

__device__ __host__ inline float test_cuda_function_4(float a, float b) {return (a-b) + (b-a);}
...
int main(int argc,char **argv)
{
...

  {
    ...
    q.submit([&](sycl::handler &h) {
      ...
      h.parallel_for(r1, [=](sycl::id<1> i) {
        d_idx[i] = test_cuda_function_1();
      });
    });
  ...
  cudaMemcpy(result.data(), cuda_kern_result, n*sizeof(int), cudaMemcpyDeviceToHost);
